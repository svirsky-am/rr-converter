...
mod tests {
    ...
    impl TestConstants {
        ...
        pub fn create_output_dir(&self) {
            let output_dir: &PathBuf  = &self.get_output_path();
            if !output_dir.as_path().exists() {
                    let result_of_create_dir= std::fs::create_dir_all(output_dir);
    ...
    #[test]
    fn test_parse_camt053_to_some_formats() {

        &TestConstants.create_output_dir();
        // let output_dir: &PathBuf  = &TestConstants::get_output_path();
        // if !output_dir.as_path().exists() {
        //         let result_of_create_dir= std::fs::create_dir_all(output_dir);
        //         result_of_create_dir.unwrap();
        //     // }
        // }
