...
pub fn parse_mt940_alt(input: &str) -> Vec<Mt940Message> {
    ...
    for cap in re_msg_all.captures_iter(input) {
        ...
        messages.push(Mt940Message {
            transaction_reference: field_map.get(":20:").cloned().unwrap_or_default(),
            account_identification: field_map.get(":25:").cloned().unwrap_or_default(),
...
fn parse_61(s: &str) -> Transaction {
    ...
    Transaction {
        ...
        transaction_type,
        reference,
        description: String::new(), // filled later
    }
}
